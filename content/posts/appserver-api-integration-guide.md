...
![Security-Overview-Flow](./diagrams/security_overview_flow.svg)
...
![Sign-Validation-Flow](./diagrams/sign_validation_flow.svg)
...
![Nonce-Sequence](./diagrams/nonce_sequence.svg)
...
![Session-Lifecycle](./diagrams/session_lifecycle.svg)

单次请求内部校验顺序如下图所示：

![Single-Request-Check](./diagrams/single_request_check.svg)
...
![Recaptcha-Flow](./diagrams/recaptcha_flow.svg)
...
![TwoFactor-Login-Flow](./diagrams/twofactor_login_flow.svg)
...
![TwoFactor-Trusted-Device-Flow](./diagrams/twofactor_trusted_device_flow.svg)
...
![TwoFactor-Stepup-Flow](./diagrams/twofactor_stepup_flow.svg)
...
![Frontend-TwoFactor-Retry-Flow](./diagrams/frontend_twofactor_retry_flow.svg)

如果需要从实现角度理解 `frontend/src/stores/request.ts` 的内部转移关系，建议拆成“主请求链路”和“2FA 重试链路”两张图分别看：

**主请求链路：**

![Frontend-Request-Base-State-Machine](./diagrams/frontend_request_base_state_machine.svg)

**2FA 重试链路：**

**2FA 重试准备阶段：**

![Frontend-Request-Retry-Prep-State-Machine](./diagrams/frontend_request_retry_prep_state_machine.svg)

**2FA 重试执行阶段：**

![Frontend-Request-Retry-Execute-State-Machine](./diagrams/frontend_request_retry_execute_state_machine.svg)
...
![Security-User-Login-Entry-Flow](./diagrams/security_user_login_entry_flow.svg)

**阶段 B：已登录请求、JWT、防重放、权限/限流/IP 校验**

![Security-User-Authenticated-Request-Flow](./diagrams/security_user_authenticated_request_flow.svg)

**阶段 C：高风险 step-up 2FA 与单次重试**

![Security-User-Stepup-Retry-Flow](./diagrams/security_user_stepup_retry_flow.svg)

**程序化接入总链路：**

![Security-Full-Chain-Programmatic](./diagrams/security_full_chain_programmatic_flow.svg)

**代理中继接入总链路：**

![Security-Full-Chain-Relay](./diagrams/security_full_chain_relay_flow.svg)
...
![JWT-Login-Active-Session-Flow](./diagrams/jwt_login_active_session_flow.svg)

**B. Access Token 过期后的刷新恢复：**

![JWT-Refresh-Recovery-Flow](./diagrams/jwt_refresh_recovery_flow.svg)
